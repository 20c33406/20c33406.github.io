...
function renderObjects() {
  ...
  for (let i = 0; i < objects.length; i++) {
    if (objects[i].mass != 0) {
      ...
      rocket.arc(0, 0, object.size, 0, Math.PI * 2);
      rocket.fill();
      rocket.restore();
    }
  }
...
function createGame() {
  rocket.clearRect(0, 0, canvas.width, canvas.height);
  objects = [];
  let nBodies = astCount.value;
  let dist = 100 * astCount.value;
  scale = 400 / dist;
  let angle = 0;
  let velocity = astSpeed * 5;
  for (let i = 0; i < nBodies; i++) {
    ...
    objects.push(new object(x, y, 10, 10e12, x_v, y_v));
...
function createGame() {
  ...
  let angle = 0
  for (let i = 0; i < 3; i++) {
    let x = 1000 * Math.cos(angle);
    let y = 1000 * Math.sin(angle);

    let x_v = velocity * Math.cos(angle + Math.PI / 2);
    let y_v = velocity * Math.sin(angle + Math.PI / 2);
    objects.push(new object(x, y, 10, 1e9, x_v, y_v));
    angle += (Math.PI * 2) / 3;
  }
  */
/*
  objects.push(
    new object(
      ...
      10,
      1e12,
      (astSpeed / 20) * Math.cos(Math.PI / 2),
      (astSpeed / 20) * Math.sin(Math.PI / 2),
    ),
  );
  objects.push(
    new object(
      1000 * Math.cos((2 * Math.PI) / 3),
      1000 * Math.sin((2 * Math.PI) / 3),
      10,
      1e12,
      (astSpeed / 20) * Math.cos(Math.PI / 2 + (2 * Math.PI) / 3),
      (astSpeed / 20) * Math.sin(Math.PI / 2 + (2 * Math.PI) / 3),
    ),
  );
  objects.push(
    new object(
      1000 * Math.cos((4 * Math.PI) / 3),
      1000 * Math.sin((4 * Math.PI) / 3),
      10,
      1e12,
      (astSpeed / 20) * Math.cos(Math.PI / 2 + (4 * Math.PI) / 3),
      (astSpeed / 20) * Math.sin(Math.PI / 2 + (4 * Math.PI) / 3),
    ),
  );
  */
/*
  objects.push(new object(0, 0, 10, 1e9, 0, 0));
  objects.push(new object(1000, 0, 10, 1e9, 0, 2 * astSpeed));
  objects.push(new object(-1000, 0, 10, 1e9, 0, -2 * astSpeed));
  */
/*
